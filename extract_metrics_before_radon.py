...
cc_data  = rodar_radon(["radon", "cc",  "./piccolo", "-j"])
print("Rodando radon mi...")
mi_data  = rodar_radon(["radon", "mi",  "./piccolo", "-j"])
print("Rodando radon hal...")
hal_data = rodar_radon(["radon", "hal", "./piccolo", "-j"])
print("Rodando radon raw...")
raw_data = rodar_radon(["radon", "raw", "./piccolo", "-j"])
...
# Halstead por arquivo e por função
vistos_hal = set()
...
def salvar_csv(nome, linhas, ordenar_por=None):
    ...
    print(f"{len(linhas):>5} linhas → {caminho}")
